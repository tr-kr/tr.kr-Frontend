...
const FirstQualifer = ({ setModalOpen, modalState, team }: modalProps) => {
  return (
    <div className="flex flex-col justify-between items-center relative h-[90px]">
      <div
        ...
        onClick={() =>
          setModalOpen({ ...modalState, open: true, callId: 'first' })
        }
      >
        {team.first}
      </div>
      <div
        ...
        onClick={() =>
          setModalOpen({ ...modalState, open: true, callId: 'second' })
        }
      >
        {team.second}
      </div>
